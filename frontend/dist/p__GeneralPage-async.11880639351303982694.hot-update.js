globalThis.makoModuleHotUpdate('p__GeneralPage', {
        ...
        "src/pages/GeneralPage.tsx": function(module, exports, __mako_require__) {
            ...
            const GeneralPage = ()=>{
                ...
                (0, _react.useEffect)(()=>{
                    ...
                    const nodeWidth = 80; // 节点占用宽度（包括间距）
                    const nodesWithPosition = processedData.nodes.map((node)=>{
                        const layer = node.layer || 0;
                        let x = leftMargin + availableWidth / 2;
                        let y = getYByLayer(layer);
                        // 事件层（layer 1）使用三层树状布局
                        if (layer === 1) {
                            const layerHeight = LAYER_HEIGHTS[1];
                            const subLayerHeight = layerHeight / 3;
                            const layerStartY = LAYER_HEIGHTS[0] + LAYER_GAP; // 事件层起始Y坐标
                            if (node.typeKey === 'EVENT') {
                                // 主事件在事件层顶部，均匀分布
                                const mainEvents = eventLayerNodes.main;
                                const index = mainEvents.findIndex((n)=>n.id === node.id);
                                const total = mainEvents.length;
                                const spacing = total > 1 ? availableWidth / (total + 1) : 0;
                                x = total > 1 ? leftMargin + (index + 1) * spacing : leftMargin + availableWidth / 2;
                                y = layerStartY + subLayerHeight * 0.5;
                            } else if (node.typeKey === 'SUB_EVENT') {
                                // 子事件：找到父主事件，在其下方水平分散
                                const parentId = childToParent.get(node.id);
                                if (parentId) {
                                    const parentNode = processedData.nodes.find((n)=>n.id === parentId);
                                    if (parentNode && parentNode.typeKey === 'EVENT') {
                                        // 获取父节点的X坐标
                                        const parentIndex = eventLayerNodes.main.findIndex((n)=>n.id === parentId);
                                        const totalParents = eventLayerNodes.main.length;
                                        const parentSpacing = totalParents > 1 ? availableWidth / (totalParents + 1) : 0;
                                        const parentX = totalParents > 1 ? leftMargin + (parentIndex + 1) * parentSpacing : leftMargin + availableWidth / 2;
                                        // 获取同一父节点的所有子节点
                                        const siblings = parentToChildren.get(parentId) || [];
                                        const siblingIndex = siblings.indexOf(node.id);
                                        const siblingCount = siblings.length;
                                        // 在父节点周围分散子节点
                                        if (siblingCount === 1) x = parentX;
                                        else {
                                            const spreadWidth = Math.min(nodeWidth * siblingCount * 1.2, availableWidth / totalParents);
                                            const siblingSpacing = spreadWidth / (siblingCount + 1);
                                            x = parentX - spreadWidth / 2 + (siblingIndex + 1) * siblingSpacing;
                                        }
                                    }
                                }
                                y = layerStartY + subLayerHeight * 1.5;
                            } else if (node.typeKey === 'TIME') {
                                // 时间节点：找到父子事件，在其下方水平分散
                                const parentId = childToParent.get(node.id);
                                if (parentId) {
                                    const parentNode = processedData.nodes.find((n)=>n.id === parentId);
                                    if (parentNode && parentNode.typeKey === 'SUB_EVENT') {
                                        // 获取父子事件的X坐标（需要先计算子事件的位置）
                                        const grandParentId = childToParent.get(parentId);
                                        let parentX = leftMargin + availableWidth / 2;
                                        if (grandParentId) {
                                            const grandParentNode = processedData.nodes.find((n)=>n.id === grandParentId);
                                            if (grandParentNode && grandParentNode.typeKey === 'EVENT') {
                                                // 获取主事件的X坐标
                                                const grandParentIndex = eventLayerNodes.main.findIndex((n)=>n.id === grandParentId);
                                                const totalGrandParents = eventLayerNodes.main.length;
                                                const grandParentSpacing = totalGrandParents > 1 ? availableWidth / (totalGrandParents + 1) : 0;
                                                const grandParentX = totalGrandParents > 1 ? leftMargin + (grandParentIndex + 1) * grandParentSpacing : leftMargin + availableWidth / 2;
                                                // 计算子事件的X坐标（与子事件布局逻辑相同）
                                                const subEventSiblings = parentToChildren.get(grandParentId) || [];
                                                const subEventIndex = subEventSiblings.indexOf(parentId);
                                                const subEventCount = subEventSiblings.length;
                                                if (subEventCount === 1) parentX = grandParentX;
                                                else {
                                                    const subEventSpreadWidth = Math.min(nodeWidth * subEventCount * 1.2, availableWidth / totalGrandParents);
                                                    const subEventSpacing = subEventSpreadWidth / (subEventCount + 1);
                                                    parentX = grandParentX - subEventSpreadWidth / 2 + (subEventIndex + 1) * subEventSpacing;
                                                }
                                            }
                                        }
                                        // 获取同一子事件的所有时间节点
                                        ...
                                        if (siblingCount === 1) x = parentX;
                                            ...
                                            // 时间节点的分散宽度相对较小
                                            const spreadWidth = Math.min(nodeWidth * siblingCount * 0.8, nodeWidth * 3);
                                            const siblingSpacing = spreadWidth / (siblingCount + 1);
                                            x = parentX - spreadWidth / 2 + (siblingIndex + 1) * siblingSpacing;
                                        }
                                    }
                                }
                                y = layerStartY + subLayerHeight * 2.5;
                            }
                        } else if (layer === 2) {
                            const layerHeight = LAYER_HEIGHTS[2];
                            const subLayerHeight = layerHeight / 2;
                            const layerStartY = LAYER_HEIGHTS[0] + LAYER_GAP + LAYER_HEIGHTS[1] + LAYER_GAP; // 特征层起始Y坐标
                            if (node.typeKey === 'RiskFactor') {
                                // 风险因子在特征层顶部，均匀分布
                                const factors = featureLayerNodes.factor;
                                const index = factors.findIndex((n)=>n.id === node.id);
                                const total = factors.length;
                                const spacing = total > 1 ? availableWidth / (total + 1) : 0;
                                x = total > 1 ? leftMargin + (index + 1) * spacing : leftMargin + availableWidth / 2;
                                y = layerStartY + subLayerHeight * 0.5;
                            } else if (node.typeKey === 'RiskFeature') {
                                // 风险特征：找到父风险因子，在其下方水平分散
                                const parentId = childToParent.get(node.id);
                                if (parentId) {
                                    const parentNode = processedData.nodes.find((n)=>n.id === parentId);
                                    if (parentNode && parentNode.typeKey === 'RiskFactor') {
                                        // 获取父节点的X坐标
                                        const parentIndex = featureLayerNodes.factor.findIndex((n)=>n.id === parentId);
                                        const totalParents = featureLayerNodes.factor.length;
                                        const parentSpacing = totalParents > 1 ? availableWidth / (totalParents + 1) : 0;
                                        const parentX = totalParents > 1 ? leftMargin + (parentIndex + 1) * parentSpacing : leftMargin + availableWidth / 2;
                                        // 获取同一父节点的所有子节点
                                        const siblings = parentToChildren.get(parentId) || [];
                            ...
                            // 其他层使用水平分布
                            const nodesInLayer = layerNodesMap[layer] || [];
                            ...
                            x = total > 1 ? leftMargin + (nodeIndex + 1) * nodeSpacing : leftMargin + availableWidth / 2;
                            y = getYByLayer(layer);
                        }
                        return {
                            ...node,
    ...
    runtime._h = '14928454263692940727';
...
//# sourceMappingURL=p__GeneralPage-async.11880639351303982694.hot-update.js.map