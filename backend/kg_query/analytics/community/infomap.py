"""Infomap algorithm via python-igraph.

Uses information-theoretic random walks to detect communities.
Particularly good at finding small, tight communities in directed networks.
"""
...
class InfomapAlgorithm(BaseCommunityAlgorithm):
    name = "infomap"
    label = "Infomap"
    description = "信息论随机游走社区检测，擅长发现小规模紧密社区"
    complexity = "O(m)"
    ...
    def detect(
        ...
        try:
            ...
            logger.exception("Infomap edge extraction failed")
            return [], 0.0
        ...
        # Build igraph Graph (directed for Infomap to leverage flow)
        g = ig.Graph(n=n, edges=edge_pairs, directed=True)
        ...
        try:
            ...
            logger.exception("Infomap detection failed")
            return [], 0.0
        ...
        try:
            ...
            logger.exception("Label resolution failed in Infomap")
