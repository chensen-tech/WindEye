"""Leiden algorithm via python-igraph.

Uses the Leiden algorithm (Traag et al., 2019) which guarantees
well-connected communities through refinement steps.
"""
...
class LeidenAlgorithm(BaseCommunityAlgorithm):
    name = "leiden"
    label = "Leiden"
    description = "Leiden算法 — 模块度优化+精炼步骤，社区质量高"
    ...
    def detect(
        ...
        try:
            ...
            logger.exception("Leiden edge extraction failed")
            return [], 0.0
        ...
        try:
            ...
            logger.exception("Leiden detection failed")
            return [], 0.0

        modularity = partition.q if hasattr(partition, 'q') else g.modularity(partition)
        ...
        try:
            ...
            logger.exception("Label resolution failed in Leiden")
